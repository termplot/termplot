...
if [ "$(uname -s)" != "Darwin" ]; then
  echo "error: Ghostty screenshot probing is currently macOS-only" >&2
  exit 1
fi

for required in open screencapture sips; do
...
visible_seconds="${GHOSTTY_SCREENSHOT_PROBE_VISIBLE_SECONDS:-8}"
timeout_seconds="${GHOSTTY_SCREENSHOT_PROBE_TIMEOUT_SECONDS:-20}"
tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/termplot-ghostty-screenshot-probe.XXXXXX")"
artifact_dir="${GHOSTTY_SCREENSHOT_PROBE_ARTIFACT_DIR:-/tmp}"
artifact="$artifact_dir/termplot-ghostty-screenshot-$(basename "$tmpdir").png"
capture_x="${GHOSTTY_SCREENSHOT_PROBE_X:-80}"
capture_y="${GHOSTTY_SCREENSHOT_PROBE_Y:-120}"
capture_width="${GHOSTTY_SCREENSHOT_PROBE_WIDTH:-1200}"
capture_height="${GHOSTTY_SCREENSHOT_PROBE_HEIGHT:-800}"
window_columns="${GHOSTTY_SCREENSHOT_PROBE_COLUMNS:-100}"
window_rows="${GHOSTTY_SCREENSHOT_PROBE_ROWS:-36}"
marker="$tmpdir/marker"
log="$tmpdir/render.log"
input="$tmpdir/input.txt"
image="$tmpdir/test-image.ppm"
pre_pids="$tmpdir/pre-pids.txt"
title="TermPlot screenshot probe $(basename "$tmpdir")"
...
echo "pass: captured Ghostty render screenshot and cleaned up"
