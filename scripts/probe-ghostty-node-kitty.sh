...
if [ "$(uname -s)" != "Darwin" ]; then
  echo "error: Ghostty Node.js Kitty probing is currently macOS-only" >&2
  exit 1
fi
...
visible_seconds="${GHOSTTY_NODE_KITTY_PROBE_VISIBLE_SECONDS:-8}"
timeout_seconds="${GHOSTTY_NODE_KITTY_PROBE_TIMEOUT_SECONDS:-20}"
tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/termplot-ghostty-node-kitty-probe.XXXXXX")"
artifact_dir="${GHOSTTY_NODE_KITTY_PROBE_ARTIFACT_DIR:-/tmp}"
artifact="$artifact_dir/termplot-ghostty-node-kitty-$(basename "$tmpdir").png"
capture_x="${GHOSTTY_NODE_KITTY_PROBE_X:-80}"
capture_y="${GHOSTTY_NODE_KITTY_PROBE_Y:-120}"
capture_width="${GHOSTTY_NODE_KITTY_PROBE_WIDTH:-1200}"
capture_height="${GHOSTTY_NODE_KITTY_PROBE_HEIGHT:-800}"
window_columns="${GHOSTTY_NODE_KITTY_PROBE_COLUMNS:-100}"
window_rows="${GHOSTTY_NODE_KITTY_PROBE_ROWS:-36}"
threshold="${GHOSTTY_NODE_KITTY_PROBE_THRESHOLD:-20}"
crop_width="${GHOSTTY_NODE_KITTY_PROBE_CROP_WIDTH:-520}"
crop_height="${GHOSTTY_NODE_KITTY_PROBE_CROP_HEIGHT:-520}"
crop_x="${GHOSTTY_NODE_KITTY_PROBE_CROP_X:-0}"
repo_root="$(pwd -P)"
fixture="${GHOSTTY_NODE_KITTY_FIXTURE:-$repo_root/scripts/fixtures/node-kitty-direct.js}"
marker="$tmpdir/marker"
log="$tmpdir/render.log"
input="$tmpdir/input.txt"
kitty_output="$tmpdir/node-kitty-output.bin"
pre_pids="$tmpdir/pre-pids.txt"
title="TermPlot Node Kitty probe $(basename "$tmpdir")"

if [ ! -f "$fixture" ]; then
  echo "error: Node.js Kitty fixture not found: $fixture" >&2
  exit 1
fi
...
assert_kitty_output() {
  if [ ! -s "$kitty_output" ]; then
    echo "error: missing or empty Node.js Kitty output artifact: $kitty_output" >&2
    exit 1
  fi

  if ! perl -0777 -ne 'exit(!(/\x1b_G/ && /\x1b\\/ && /f=24/ && /s=64/ && /v=64/ && !/File=/))' "$kitty_output"; then
    echo "error: Node.js output did not prove Kitty APC bytes" >&2
    exit 1
  fi
}
...
kitty_output_q="$(shell_quote "$kitty_output")"
visible_seconds_q="$(shell_quote "$visible_seconds")"

cat > "$input" <<EOF
run_termplot_node_kitty_probe() {
{
...
printf 'started\n' > $marker_q
node $fixture_q --capture=$kitty_output_q 2>> $log_q
node_status=\$?
...
echo "node_kitty_probe_tmpdir=$tmpdir"
echo "ghostty_app=$ghostty_app"
echo "fixture=$fixture"
echo "marker=$marker"
echo "log=$log"
echo "input=$input"
echo "node_kitty_output=$kitty_output"
echo "title=$title"
  ...
  if [ -f "$marker" ]; then
    ...
    case "$state" in
      rendered)
        ...
        ;;
      failed)
        echo "error: Node.js Kitty renderer failed inside Ghostty" >&2
        echo "render_log:" >&2
        sed 's/^/  /' "$log" >&2
        exit 1
        ;;
...
if [ "$state" != "rendered" ]; then
  echo "error: timed out waiting for Ghostty Node.js Kitty render marker" >&2
...
echo "render_log:"
sed 's/^/  /' "$log"
echo "node_kitty_protocol_attribution=kitty-apc"
echo "node_kitty_output_bytes=$(wc -c < "$kitty_output" | tr -d ' ')"
echo "node_kitty_apc_chunks=$(perl -0777 -ne 'print scalar(() = /\x1b_G/g)' "$kitty_output")"
echo "node_kitty_contains_iterm_file=no"
echo "screenshot=$artifact"
...
echo "pass: Ghostty rendered Node.js Kitty output, pixels matched, and cleanup completed"
