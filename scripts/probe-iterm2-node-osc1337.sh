...
if [ "$(uname -s)" != "Darwin" ]; then
  echo "error: iTerm2 Node.js OSC 1337 probing is currently macOS-only" >&2
  exit 1
fi
...
repo_root="$(pwd -P)"
fixture="${ITERM2_NODE_OSC1337_FIXTURE:-$repo_root/scripts/fixtures/node-iterm2-osc1337-direct.js}"
visible_seconds="${ITERM2_NODE_OSC1337_PROBE_VISIBLE_SECONDS:-8}"
timeout_seconds="${ITERM2_NODE_OSC1337_PROBE_TIMEOUT_SECONDS:-20}"
tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/termplot-iterm2-node-osc1337-probe.XXXXXX")"
artifact_dir="${ITERM2_NODE_OSC1337_PROBE_ARTIFACT_DIR:-/tmp}"
artifact="$artifact_dir/termplot-iterm2-node-osc1337-$(basename "$tmpdir").png"
capture_x="${ITERM2_NODE_OSC1337_PROBE_X:-80}"
capture_y="${ITERM2_NODE_OSC1337_PROBE_Y:-120}"
capture_width="${ITERM2_NODE_OSC1337_PROBE_WIDTH:-1200}"
capture_height="${ITERM2_NODE_OSC1337_PROBE_HEIGHT:-800}"
window_columns="${ITERM2_NODE_OSC1337_PROBE_COLUMNS:-100}"
window_rows="${ITERM2_NODE_OSC1337_PROBE_ROWS:-36}"
threshold="${ITERM2_NODE_OSC1337_PROBE_THRESHOLD:-20}"
crop_padding="${ITERM2_NODE_OSC1337_PROBE_CROP_PADDING:-16}"
marker="$tmpdir/marker"
log="$tmpdir/render.log"
command_script="$tmpdir/run-iterm2-node-osc1337.sh"
osc_output="$tmpdir/node-iterm2-osc1337-output.bin"
png_output="$tmpdir/node-iterm2-osc1337-image.png"
pre_pids="$tmpdir/pre-pids.txt"
pre_server_pids="$tmpdir/pre-server-pids.txt"
window_probe="$tmpdir/find-iterm-window.swift"
su_enable_automatic_checks="$tmpdir/SUEnableAutomaticChecks"
su_enable_automatic_checks_exists="$tmpdir/SUEnableAutomaticChecks.exists"
...
title="TermPlot iTerm2 OSC1337 probe $(basename "$tmpdir")"

if [ ! -f "$fixture" ]; then
  echo "error: Node.js iTerm2 fixture not found: $fixture" >&2
  exit 1
fi
...
assert_osc1337_output() {
  if [ ! -s "$osc_output" ]; then
    echo "error: missing or empty Node.js OSC 1337 output artifact: $osc_output" >&2
    exit 1
  fi

  if ! perl -0777 -ne 'exit(!(/\x1b\]1337;File=/ && /inline=1/ && !/\x1b_G/))' "$osc_output"; then
    echo "error: Node.js output did not prove OSC 1337 bytes" >&2
    exit 1
  fi
}
...
sleep 0.8
node $fixture_q --capture=$osc_output_q --png=$png_output_q 2>> $log_q
node_status=\$?
printf 'node_status=%s\n' "\$node_status" >> $log_q
date '+node_done_at=%Y-%m-%dT%H:%M:%S%z' >> $log_q
...
echo "marker=$marker"
echo "log=$log"
echo "node_iterm2_output=$osc_output"
echo "node_iterm2_png=$png_output"
echo "title=$title"
  ...
  if [ -f "$marker" ]; then
    ...
    case "$state" in
      rendered)
        ...
        ;;
      failed)
        echo "error: Node.js OSC 1337 renderer failed inside iTerm2" >&2
        echo "render_log:" >&2
        sed 's/^/  /' "$log" >&2
        exit 1
        ;;
...
if [ "$state" != "rendered" ]; then
  echo "error: timed out waiting for iTerm2 Node.js OSC 1337 render marker" >&2
...
echo "render_log:"
sed 's/^/  /' "$log"
echo "node_iterm2_protocol_attribution=osc1337-file"
echo "node_iterm2_output_bytes=$(wc -c < "$osc_output" | tr -d ' ')"
echo "node_iterm2_contains_osc1337_file=yes"
echo "node_iterm2_contains_kitty_apc=no"
echo "screenshot=$artifact"
...
echo "pass: iTerm2 rendered Node.js OSC 1337 output, pixels matched, and cleanup completed"
